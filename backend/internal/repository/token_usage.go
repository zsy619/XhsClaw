...
func (r *TokenUsageRepository) GetStatsByUserID(userID uint) (*model.TokenUsageStats, error) {
	...
	err := r.db.Model(&model.TokenUsage{}).
		...
		Select("COUNT(*) as total_requests, COALESCE(SUM(prompt_tokens), 0) as total_prompt_tokens, COALESCE(SUM(completion_tokens), 0) as total_completion_tokens, COALESCE(SUM(total_tokens), 0) as total_tokens, COALESCE(SUM(cost), 0) as total_cost").
		Scan(&stats).Error
